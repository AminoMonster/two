...
class Setup
{
    ...
    public static function getLanguages()
    {
        ...
        if (!empty($langs)) {
            foreach ($langs as $lang_code) {
                $languages[$lang_code] = App::instance()->t($lang_code);
            }
        }
    ...
    /**
     * Enables live mode on store
     *
     * @return bool Always true
     */
    public function enableLiveMode()
    {
        db_query("UPDATE ?:settings_objects SET value = ?s WHERE name = ?s", 'N', 'store_optimization_dev');
    ...
    public function setupThemes()
    {
        ...
        if (!empty($this->_cart_settings['theme_name'])) {
            if (fn_allowed_for('ULTIMATE')) {
                fn_install_theme($company_1_layout, $this->_cart_settings['theme_name'], 1);

                if ($this->_install_demo) {
                    fn_install_theme($company_2_layout, $this->_cart_settings['theme_name'], 2);
                }
            } else {
                fn_install_theme($company_1_layout, $this->_cart_settings['theme_name']);
            }
    ...
    public function writeConfig()
    {
        ...
        if (!empty($config_contents)) {
            ...
            $config = array(
                ...
                'http_host' => fn_clean_url($this->_server_settings['http_host']),
                'http_path' => fn_clean_url($this->_server_settings['http_path']),
                'https_host' => fn_clean_url( $this->_server_settings['https_host']),
                'https_path' => fn_clean_url($this->_server_settings['https_path']),
    ...
    public function setupLanguages($demo)
    {
        ...
        if (!empty($languages)) {
            ...
            // some block data were saved in lang dumps, so now we can install blocks for demo company
            if (fn_allowed_for('ULTIMATE') && $demo) {
                $this->_setupDemoCompanyBlocks();
            }
    ...
    public function setupCompanies()
    {
        ...
        if (fn_allowed_for('ULTIMATE')) {
            ...
            if ($this->_install_demo) {
                $this->_updateCompanyURL(
                    $url . '/' . self::DEMO_COMPANY_URL,
                    $secure_url . '/' . self::DEMO_COMPANY_URL,
                    2
                );
            }
        }
    ...
    /**
     * Imports blocks to demo company from default company
     *
     * @return bool Always true
     */
    private function _setupDemoCompanyBlocks()
    {
        $from = Layout::instance(1)->get(); // default layout for company ID == 1
        $to = Layout::instance(2)->get(); // default layout for company ID == 2

        Location::instance($from['layout_id'])->copy($to['layout_id']);

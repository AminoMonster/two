...
class App
{
    ...
    const BASE_THEME = 'basic';

    /**
     * Type of run. If true application runed from console
     *
     * @var bool $_console
     */
    ...
    public function init($params = array())
    {
        ...
        if (!empty($params['init_admin_area'])) {
            define('AREA', 'A');
        ...
        $classLoader->add('Tygh', $base_path . '/app');
    ...
    {
        $mode = preg_replace('/_([a-zA-Z])/e' , "strtoupper('\\1')", $mode);
     ...
     * @return bool   always true
     */
    public function setNotification($type, $title, $message, $add_to_log = false, $section_id = 'general')
    {
        ...
        fn_set_notification($type, $title, $message, 'K', $section_id);
     ...
     *
     * @return string Formatted product name (E.g.: CS-Cart Ultimate)
     */
    public function getProductName()
    {
        if (PRODUCT_EDITION == 'MULTIVENDOR') {
            $product_type = 'Multi-Vendor';
        } elseif (PRODUCT_EDITION == 'ULTIMATE') {
            $product_type = 'Ultimate';
        } else {
            $product_type = 'Professional';
        }

        return 'CS-Cart ' . $product_type;
    }
    ...
    {
        $lang_files = fn_get_dir_contents(Registry::get('config.dir.install') . 'langs' , false, true, '.json');
    ...
    private function _loadLanguageVariables()
    {
        ...
        if (empty($this->_lang_data[$lang_code])) {
            $json_lang_vars = file_get_contents(Registry::get('config.dir.install') . '/langs/' . $lang_code . '.json');
            $this->_lang_data[$lang_code] = json_decode($json_lang_vars, true);
    ...
    public function connectToDB($host, $name, $user, $password, $table_prefix, $database_backend, $names = 'utf8')
    {
        ...
        if ($this->_db_connection == null) {
            ...
            if (!$connected) {
                $this->setNotification('E', '', $this->t('could_not_connect_to_database'));
            } elseif (!Database::changeDb($name)) {
                if (!Database::createDb($name)) {
                    $this->setNotification('E', '', $this->t('could_not_create_database'));
                    $connected = false;
                } else {
